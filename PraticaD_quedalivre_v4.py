...
dct = {"assunto": assunto, "altura": alt, "coords": []}


# Calculando o tempo e a velocidade a partir da lista ly
for y in ly:
    t = (2 * (alt - y) / g)**0.5
    v = g * t

    # Adicionando os valores as listas criadas
    dct["coords"].append([y, t, v])

# Por último, imprimimos os valores obtidos na devida formatação
for i in range(0, npts): 
    print("t: {:.3} s\t y: {:.3} m\t v: {:.3} m/s".format(dct["coords"][i][1], dct["coords"][i][0], dct["coords"][i][2]))
